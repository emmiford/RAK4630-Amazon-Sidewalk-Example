...
## Acceptance Criteria
- [ ] `app.c` is ≤250 lines and only handles boot sequence + timer
- [ ] `platform_shell.c` contains all platform shell commands (no duplicates)
- [ ] `sidewalk_dispatch.c` contains all Sidewalk event handlers
- [ ] `sid_shell.c` deleted (functionality consolidated into platform_shell.c)
- [ ] No duplicate `cmd_sid_status()` implementations
...
- [ ] Host-side tests pass (no app-layer changes)

## Deliverables
- New: `src/platform_shell.c` (~200 lines), `src/sidewalk_dispatch.c` (~150 lines)
- Modified: `src/app.c` (reduced from 652 to ~200 lines)
