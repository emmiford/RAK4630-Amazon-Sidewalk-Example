...
static void state_notify_data(void *o)
{
	...
	case APP_EVENT_NOTIFY_SENSOR: {
		LOG_INF("APP_EVENT_NOTIFY_SENSOR - EVSE Telemetry");

		/* Read EVSE sensor data */
		evse_payload_t evse_data = rak_sidewalk_get_payload();

		/* For demo compatibility, send J1772 state as "temperature" value */
		/* This maps: State A=0 (disconnected) through State E=4 (error) */
		float evse_state_as_temp = (float)evse_data.j1772_state;

		/* Prepare message using existing demo framework */
		struct sid_demo_action_notification notify_evse = {
			.gps_time_in_seconds = time_in_sec_get(),
			.link_type = last_link_mask_get(),
			.temp_sensor = SID_DEMO_TEMPERATURE_SENSOR_UNITS_CELSIUS,
			.temperature = evse_state_as_temp,
		...
		/* Serialize and send */
		sid_parse_state_init(&state, msg_buffer, sizeof(msg_buffer));
		sid_demo_app_action_notification_serialize(&state, &notify_evse);
		if (state.ret_code != SID_ERROR_NONE) {
			LOG_ERR("Notify EVSE serialize failed -%d", state.ret_code);
			break;
		}

		err = app_tx_demo_msg_send(&state, msg_buffer, &notify_desc, &notify_sid_desc);
		if (err) {
			LOG_ERR("Notify EVSE send failed %d", err);
		}

		LOG_INF("EVSE notify: state=%d, pilot=%dmV, current=%dmA, therm=0x%02x",
			evse_data.j1772_state, evse_data.j1772_mv,
			evse_data.current_ma, evse_data.thermostat_flags);
	} break;
	case APP_EVENT_RESP_LED_ON: {
		// Read led status
