...
| Done | TASK-026 | Boot path + app discovery tests — 13 tests (Eero) |
...
### TASK-026: Add app discovery and boot path tests — DONE (Eero)

## Status: DONE (2026-02-13, Eero)
**Session**: `4ec23ff8-941a-43e9-8467-adbba0158176` | **Branch**: `feature/app-boot-tests`

Extracted `app_boot_validate()` from inline checks in `discover_app_image()` into standalone `app_boot.c` module (~20 lines, no Zephyr deps). 13 Unity tests in `tests/app/test_app_boot.c` covering magic/version validation. Refactored `app.c` from 30 lines of inline checks to a single `app_boot_validate()` call. All 10 C test suites (including new) and 32 app-side tests pass.

## Branch & Worktree Strategy
**Base Branch**: `main`
- Branch: `feature/app-boot-tests`
