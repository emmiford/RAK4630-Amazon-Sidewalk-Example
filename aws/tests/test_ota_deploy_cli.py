"""Tests for OTA CLI pure functions — CRC32, delta chunks, formatting."""
...
from protocol_constants import crc32  # noqa: E402
...
class TestCrc32:
    ...
    def test_different_data_different_crc(self):
        assert crc32(b"\x00" * 60) != crc32(b"\xFF" * 60)
...
class TestComputeDeltaChunks:
    def test_identical_firmware_no_changes(self):
        fw = b"\x00" * 60
        baseline = b"\x00" * 60
        changed = ota.compute_delta_chunks(baseline, fw, 15)
        assert changed == []
    ...
    def test_one_chunk_changed(self):
        ...
        changed = ota.compute_delta_chunks(baseline, bytes(fw), 15)
        assert changed == [0]
    ...
    def test_empty_baseline_all_changed(self):
        fw = b"\x00" * 30
        changed = ota.compute_delta_chunks(b"", fw, 15)
        assert changed == [0, 1]
...
class TestFormatDuration:
    def test_seconds_only(self):
        assert ota.format_duration(45) == "45s"

    def test_minutes_and_seconds(self):
        assert ota.format_duration(125) == "2m 5s"

    def test_hours_and_minutes(self):
        assert ota.format_duration(3720) == "1h 2m"

    def test_zero(self):
        assert ota.format_duration(0) == "0s"

    def test_exactly_one_minute(self):
        assert ota.format_duration(60) == "1m 0s"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if app_bin_size is not None:
                ...
                def mock_exists(path):
                    if path.endswith(os.path.join(ota.BUILD_APP_DIR, "app.bin")):
                        return True
                ...
                with patch("subprocess.run", side_effect=fake_run), \
                    ...
                    result = ota.pyocd_dump(0x90000, 256 * 1024, dump_path)
            else:
                with patch("subprocess.run", side_effect=fake_run):
                    result = ota.pyocd_dump(0x90000, 256 * 1024, dump_path)
