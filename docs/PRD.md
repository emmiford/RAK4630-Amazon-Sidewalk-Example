...
| TBD | TBD | W-out relay | Pass-through relay on heat call: when open, blocks W-in signal from reaching compressor contactor via W-out. Controlled by hardware interlock in v1.0; firmware control in v1.1 | DESIGNED (HW v1.0) |
...
- Three relay outputs:
  - **Charge block relay** (PRD 2.4): Controls the J1772 pilot spoof circuit on the PILOT-out line. When the MCU drives charge_block HIGH, the relay engages and inserts ~900 ohm resistance on PILOT-out, making the charger see J1772 State B (connected, not ready) and stop supplying power. When charge_block is LOW (or MCU loses power), the hardware safety gate controls the relay independently. GPIO IO1 (nRF52840: P0.17) on WisBlock prototype, active high = blocking. Production PCB pin TBD.
